#!/bin/sh
# NilWM installer for Alpine Linux
# Installs Xorg, build tools, compiles and installs NilWM
# Usage: doas sh install-alpine.sh
set -e

REPO_URL="https://github.com/maximofernandezriera/nilwm.git"
TIMESTAMP=$(date +%s)

echo "=== NilWM Installer (Alpine) ==="

# ── Must be root ──
if [ "$(id -u)" -ne 0 ]; then
    echo "ERROR: Run as root: doas sh $0"
    exit 1
fi
REAL_USER="${DOAS_USER:-${SUDO_USER:-$USER}}"
...
apk add -q \
    ...
    build-base \
    libx11-dev libxft-dev libxinerama-dev freetype-dev fontconfig-dev \
    font-dejavu \
    dmenu \
    git xsetroot
...
echo "  startx"
echo ""
echo "Note: On Alpine you may need to enable the 'udev' service:"
echo "  rc-update add udev"
echo "  rc-service udev start"
echo ""
